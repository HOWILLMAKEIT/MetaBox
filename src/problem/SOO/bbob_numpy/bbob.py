from .basic_problem import BBOB_Basic_Problem, GaussNoisyProblem, UniformNoisyProblem, CauchyNoisyProblem
import numpy as np
...
class F3(BBOB_Basic_Problem):
	...
	def func(self, x):
		self.FES += x.shape[0]
		z = self.scales * self.asy_transform(self.osc_transform(self.sr_func(x, self.shift, self.rotate)), beta=0.2)
...
class F4(BBOB_Basic_Problem):
	...
	def func(self, x):
		...
		even[even > 0.] *= 10.
		z *= self.scales
		return 10 * (self.dim - np.sum(np.cos(2 * np.pi * z), axis=-1)) + np.sum(z ** 2, axis=-1) + 100 * self.pen_func(
			x, self.ub) + self.bias
...
class F6(BBOB_Basic_Problem):
	...
	def __init__(self, dim, shift, rotate, bias, lb, ub):
		...
		rotate = np.matmul(np.matmul(self.rotate_gen(dim), np.diag(scales)), rotate)
		BBOB_Basic_Problem.__init__(self, dim, shift, rotate, bias, lb, ub)
	...
	def func(self, x):
		...
		z[idx] *= 100.
		return self.osc_transform(np.sum(z ** 2, -1)) ** 0.9 + self.bias
...
class _Step_Ellipsoidal(BBOB_Basic_Problem):
	...
	def __init__(self, dim, shift, rotate, bias, lb, ub):
		...
		self.Q_rotate = self.rotate_gen(dim)
		BBOB_Basic_Problem.__init__(self, dim, shift, rotate, bias, lb, ub)

	def func(self, x):
		self.FES += x.shape[0]
		z_hat = self.sr_func(x, self.shift, self.rotate)
...
class F7(_Step_Ellipsoidal):
	def boundaryHandling(self, x):
		return self.pen_func(x, self.ub)
...
class _Rosenbrock(BBOB_Basic_Problem):
	...
	def func(self, x):
		self.FES += x.shape[0]
		z = max(1., self.dim ** 0.5 / 8.) * self.sr_func(x, self.shift, self.rotate) + 1
...
class F12(BBOB_Basic_Problem):
	...
	def func(self, x):
		self.FES += x.shape[0]
		z = self.sr_func(x, self.shift, self.rotate)
		z = self.asy_transform(z, beta=self.beta)
...
class F15(BBOB_Basic_Problem):
	...
	def __init__(self, dim, shift, rotate, bias, lb, ub):
		scales = (10. ** 0.5) ** np.linspace(0, 1, dim)
		self.linearTF = np.matmul(np.matmul(rotate, np.diag(scales)), self.rotate_gen(dim))
		BBOB_Basic_Problem.__init__(self, dim, shift, rotate, bias, lb, ub)
	...
	def func(self, x):
		self.FES += x.shape[0]
		z = self.sr_func(x, self.shift, self.rotate)
		z = self.asy_transform(self.osc_transform(z), beta=0.2)
...
class F16(BBOB_Basic_Problem):
	...
	def func(self, x):
		self.FES += x.shape[0]
		z = self.sr_func(x, self.shift, self.rotate)
		z = np.matmul(self.osc_transform(z), self.linearTF.T)
		return 10 * np.power(
			np.mean(np.sum(self.aK * np.cos(np.matmul(2 * np.pi * (z[:, :, None] + 0.5), self.bK[None, :])), axis=-1),
					axis=-1) - self.f0, 3) + \
			10 / self.dim * self.pen_func(x, self.ub) + self.bias
...
class _Scaffer(BBOB_Basic_Problem):
	...
	def __init__(self, dim, shift, rotate, bias, lb, ub):
		...
		self.linearTF = np.matmul(np.diag(scales), self.rotate_gen(dim))
		BBOB_Basic_Problem.__init__(self, dim, shift, rotate, bias, lb, ub)

	def func(self, x):
		self.FES += x.shape[0]
		z = self.sr_func(x, self.shift, self.rotate)
		z = np.matmul(self.asy_transform(z, beta=0.5), self.linearTF.T)
...
class F17(_Scaffer):
	...
	def boundaryHandling(self, x):
		return 10 * self.pen_func(x, self.ub)
...
class F20(BBOB_Basic_Problem):
	...
	def func(self, x):
		...
		b = 4.189828872724339
		return b - 0.01 * np.mean(z * np.sin(np.sqrt(np.abs(z))), axis=-1) + 100 * self.pen_func(z / 100,
																								 self.ub) + self.bias
...
class _Gallagher(BBOB_Basic_Problem):
	...
	def func(self, x):
		...
		return self.osc_transform(10 - z) ** 2 + self.bias + self.boundaryHandling(x)
...
class F23(BBOB_Basic_Problem):
	...
	def func(self, x):
		...
		return res * tmp - tmp + self.pen_func(x, self.ub) + self.bias
...
class F24(BBOB_Basic_Problem):
	...
	def func(self, x):
		...
		return np.minimum(np.sum((x_hat - self.mu0) ** 2., axis=-1),
			...
			10. * (self.dim - np.sum(np.cos(2. * np.pi * z), axis=-1)) + 1e4 * self.pen_func(x, self.ub) + self.bias
