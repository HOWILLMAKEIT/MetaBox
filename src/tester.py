...
class Tester(object):
    def __init__(self, config):
        ...
        for problem in self.test_set:
            ...
            self.test_results['pr'][problem.__str__()] = {}
            self.test_results['sr'][problem.__str__()] = {}
            self.test_results['T1'][problem.__str__()] = {}
            self.test_results['T2'][problem.__str__()] = {}
            self.test_results['T0'][problem.__str__()] = 0.
            for agent_name in self.agent_name_list:
                self.test_results['cost'][problem.__str__()][agent_name] = []  # 51 np.arrays
                self.test_results['pr'][problem.__str__()][agent_name] = []  # 51 scalars
                self.test_results['sr'][problem.__str__()][agent_name] = []  # 51 scalars
                self.test_results['T1'][problem.__str__()][agent_name] = 0.
                self.test_results['T2'][problem.__str__()][agent_name] = 0.
            for optimizer in self.t_optimizer_for_cp:
                self.test_results['cost'][problem.__str__()][type(optimizer).__name__] = []  # 51 np.arrays
                self.test_results['pr'][problem.__str__()][agent_name] = []  # 51 scalars
                self.test_results['sr'][problem.__str__()][agent_name] = []  # 51 scalars
                self.test_results['T1'][problem.__str__()][type(optimizer).__name__] = 0.
                self.test_results['T2'][problem.__str__()][type(optimizer).__name__] = 0.
    ...
    def test_1(self):
        ...
        with tqdm(range(pbar_len), desc = "Testing") as pbar:
            for i, problem in enumerate(self.test_set.data):
                T0 = cal_t0(problem.dim, problem.maxfes)
                self.test_results['T0'][problem.__str__()] = T0
                for agent_id, (agent, optimizer) in enumerate(zip(self.agent_for_cp, self.l_optimizer_for_cp)):
                    ...
                    cost = meta_test_data['cost'] # 假设已经对齐不同环境的长度
                    pr = meta_test_data['pr']
                    sr = meta_test_data['sr']
                    end = time.perf_counter()
                    T2 = (end - start) * 1000  # ms
                    T1 = np.sum(meta_test_data['T1'])
                    self.test_results['cost'][problem.__str__()][self.agent_name_list[agent_id]] = np.array(cost)
                    self.test_results['pr'][problem.__str__()][self.agent_name_list[agent_id]] = np.array(pr)
                    self.test_results['sr'][problem.__str__()][self.agent_name_list[agent_id]] = np.array(sr)
                    self.test_results['T1'][problem.__str__()][self.agent_name_list[agent_id]] = T1/test_run
                    self.test_results['T2'][problem.__str__()][self.agent_name_list[agent_id]] = T2/test_run
                    # meta_test_data : {cost, fes, return} {'return': _Rs, 'cost': env_cost, 'pr': raw_PRs, 'sr': raw_SRs}
